...
const draw_all = async (msg) => {
    ...
    const height = (data.length + 1) * sectionHeight;
    ...
    data.forEach((user, index) => {
        ...
        ctx.fillText(user.username, 10, y + sectionHeight / 2);

    });
    ctx.textAlign = 'center';
    ctx.strokeStyle = `${color.fg}cc`;
    const tz = -(new Date().getTimezoneOffset() / 60);
    for (let i = 0; i < 24; i++) {
        const x = i * hourwidth;
        ctx.beginPath();
        ctx.moveTo(x, sectionHeight);
        ctx.lineTo(x, height);
        ctx.stroke();
        ctx.fillText(`${i + tz >= 24 ? i + tz - 24: i + tz}`, x + hourwidth / 2, sectionHeight / 2);
    }
